...
docker run --rm -it --add-host conjur.demo.com:$SERVICE_IP -v $(pwd)/mydata/:/root cyberark/conjur-cli:5 variable values add test-app-summon-sidecar-ajustadores-app/db-username "test_app"
docker run --rm -it --add-host conjur.demo.com:$SERVICE_IP -v $(pwd)/mydata/:/root cyberark/conjur-cli:5 variable values add test-app-summon-sidecar-ajustadores-app/db-password "5b3e5f75cb3cdc725fe40318"
docker run --rm -it --add-host conjur.demo.com:$SERVICE_IP -v $(pwd)/mydata/:/root cyberark/conjur-cli:5 variable values add test-app-summon-sidecar-ajustadores-app/url "postgresql://test-summon-sidecar-app-backend.test-app-summon-sidecar.svc.cluster.local:5432/postgres"
